...
def index(album_id=None):
    ...
    if album_type is not None and album_id is None and not album_age:
        ...
        album_id = db.execute(f'SELECT a.id FROM albums a '\
        ...
        album_count = db.execute(' SELECT id FROM albums ORDER BY RANDOM() LIMIT 1; ').fetchone()
        # random_album_id = random.randint(1, album_count['count'])
        random_album_id = album_count['id']
        return redirect(url_for('album.index', album_id=random_album_id, album_type='x'))
    ...
    album = db.execute('SELECT * FROM albums a '
                       ' WHERE id = ? LIMIT 1;', (random_album_id,)).fetchone()

    
    if album is None: # this is broken
    ...
    rating_data = db.execute('SELECT COUNT(*) AS cnt, ROUND(AVG(rating),1) AS avg ' 
        ' FROM ratings r ' 
